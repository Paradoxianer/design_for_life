...
// CLI dev script (dart run scripts/convert_gifts_csv.dart) - print() is the
// right tool here, no Flutter widget tree/app context exists to log into.
import 'dart:io';
import 'dart:convert';
...
void _convert(String locale) {
  ...
  final colMap = {
    for (var i = 0; i < rawHeader.length; i++)
      rawHeader[i].trim().toLowerCase(): i,
  };

  int getCol(List<String> keys) {
    for (final key in keys) {
      if (colMap.containsKey(key.toLowerCase())) {
  ...
  final idxQuestion = getCol([
    'frage',
    'test-frage (biblischer bezug)',
    'question',
    'text',
  ]);
  ...
  print(
    'Indices ($locale): ID:$idxGiftId, Name:$idxGiftName, Orig:$idxOriginalWord, Meaning:$idxMeaning, Refs:$idxRefs, Desc:$idxDesc, Type:$idxType, Q:$idxQuestion',
  );
  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    if (rowGiftName != null && rowGiftName.isNotEmpty) {
      String? giftId;
      if (idxGiftId != -1 &&
          idxGiftId < parts.length &&
          parts[idxGiftId].trim().isNotEmpty) {
        giftId = parts[idxGiftId].trim();
      } else {
        giftId =
            nameToIdMap[rowGiftName] ??
            nameToIdMap[rowGiftName.replaceAll(' / ', '/')] ??
            'GXX';
      }
      ...
      if (currentGift == null || currentGift['id'] != giftId) {
        currentGift = {
          ...
          'name': rowGiftName,
          'originalWord':
              idxOriginalWord != -1 && idxOriginalWord < parts.length
              ? parts[idxOriginalWord].trim()
              : '',
          'meaning': idxMeaning != -1 && idxMeaning < parts.length
              ? parts[idxMeaning].trim()
              : '',
          'bibleReferences': idxRefs != -1 && idxRefs < parts.length
              ? parts[idxRefs]
                    ...
                    .toList()
              : [],
          'description': idxDesc != -1 && idxDesc < parts.length
              ? parts[idxDesc].trim()
              : '',
          'questions': [],
        };
    ...
    if (currentGift != null) {
      final qText = idxQuestion != -1 && idxQuestion < parts.length
          ? parts[idxQuestion].trim()
          : '';
      if (qText.isNotEmpty) {
        final qTypeRaw = idxType != -1 && idxType < parts.length
            ? parts[idxType].trim().toUpperCase()
            : '';
        String finalType = 'reference';
        if (qTypeRaw == 'E' || qTypeRaw == 'EXPERIENCE') {
          finalType = 'experience';
        } else if (qTypeRaw == 'N' || qTypeRaw == 'NATURE') {
          finalType = 'nature';
        } else if (qTypeRaw == 'F' ||
            qTypeRaw == 'FRUIT' ||
            qTypeRaw == 'FEEDBACK') {
          finalType = 'feedback';
        } else if (qTypeRaw == 'R' || qTypeRaw == 'REFERENCE') {
          finalType = 'reference';
        }

        final qId =
            idxQuestionId != -1 &&
                idxQuestionId < parts.length &&
                parts[idxQuestionId].trim().isNotEmpty
  ...
  for (var gift in gifts) {
    if (gift['originalWord'].isEmpty ||
        gift['meaning'].isEmpty ||
